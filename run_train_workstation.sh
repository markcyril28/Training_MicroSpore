#!/bin/bash
#===============================================================================
# Microspore Phenotyping - YOLO Training Script (WORKSTATION VERSION)
...
#   - 16 workers (high-throughput data loading)
...
#
# Config scripts are located in: config/workstation/
# Each config should define all required parameter arrays.

CONFIG_DIR="${SCRIPT_DIR}/config/workstation"

CONFIG_SCRIPTS=(
    # Uncomment the configs you want to run:
    #"01_yolo_color_combination.sh"           # YOLO model + color mode combinations
    #"02_within_yolo_and_selected_color_combination.sh"  # Within-YOLO parameter tuning
    #"03_epoch_combination.sh"                # Epoch variations
    #"04_img_size_combination.sh"             # Image size variations
...
print_header "Microspore Phenotyping - YOLO Training (Config-Based Grid Search)"
...
if ! activate_conda_env "${ENV_NAME}"; then
    print_error "Please run setup_conda_training.sh first."
    exit 1
fi
...
print_subheader "Checking GPU Availability"
print_gpu_info
echo ""

...
ADDITIONAL_EPOCHS_LIST=(
    ...
    300                     # +300 epochs - conservative continuation (continue_3 regressed)
...
PATIENCE_LIST=(
    ...
    100                     # stable continuation window (matches prior good run behavior)
...
LR0_LIST=(
    ...
    0.00001                 # best-performing continued run used 1e-5; 5e-5 regressed
)
...
CLASS_FOCUS_FOLD_LIST=(
    ...
    4.0                     # aggressive but less overfit-prone than 5x
...
MIXUP_LIST=(
    ...
    0.15                    # reduce mixing to avoid blurring already-confusable boundaries
...
IOU_THRESHOLD_LIST=(
    0.6                     # slightly lower to increase positives/recall (reduce class→background)
...
LABEL_SMOOTHING_LIST=(
    ...
    0.1                     # softer smoothing; avoids washing out subtle class boundaries
)
